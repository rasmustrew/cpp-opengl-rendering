...
vec3 getCombinedSpecular()
{
    if (material.numSpecularTextures <= 0) {
        return vec3(1.0);
    }

    vec3 color = vec3(0.0);
    for (int i = 0; i < material.numSpecularTextures; ++i) {
        color += vec3(texture(material.specular[i], TexCoords));
    }
    return color / float(material.numSpecularTextures);
}

vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir)
{
    vec3 diffuseColor = getCombinedDiffuse();
    vec3 specularColor = getCombinedSpecular();
